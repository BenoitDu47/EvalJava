...
public class IBusinessImpl implements IBusiness {
	...
	public void addToCart(Training training) {
		Training art = cart.get(training.getIdTraining());
		if (art != null) {
	...
	public void rmFromCart(int id) {
		...
		if (training != null) {
			if (training.getQuantity() == 1)
				cart.remove(id);
			else
				training.setQuantity(training.getQuantity() - 1);
		}
	}
		...
		if (customerDao.read(idCustomer) != null) {
			...
			if (orderDao.create(order)) {
				for (Training training : cart.values()) {
					orderItemDao.create(new OrderItem(0, training.getIdTraining(), training.getQuantity(),
							training.getPrice(), order.getIdOrder()));
				}
